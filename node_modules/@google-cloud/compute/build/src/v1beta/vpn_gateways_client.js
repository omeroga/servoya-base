...
exports.VpnGatewaysClient = void 0;
const jsonProtos = require("../../protos/protos.json");
const google_gax_1 = require("google-gax");
/**
 * Client JSON configuration object, loaded from
 * `src/v1beta/vpn_gateways_client_config.json`.
 * This file defines retry strategy and timeouts for all API methods in this library.
 */
const gapicConfig = require("./vpn_gateways_client_config.json");
...
class VpnGatewaysClient {
    ...
    constructor(opts, gaxInstance) {
        ...
        this._defaults = this._gaxGrpc.constructSettings('google.cloud.compute.v1beta.VpnGateways', gapicConfig, opts.clientConfig || {}, { 'x-goog-api-client': clientHeader.join(' ') });
    ...
    initialize() {
        ...
        if (this.vpnGatewaysStub) {
            return this.vpnGatewaysStub;
        }
        // Put together the "service stub" for
        // google.cloud.compute.v1beta.VpnGateways.
        this.vpnGatewaysStub = this._gaxGrpc.createStub(this._opts.fallback ?
            this._protos.lookupService('google.cloud.compute.v1beta.VpnGateways') :
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            this._protos.google.cloud.compute.v1beta.VpnGateways, this._opts, this._providedCustomServicePath);
        // Iterate over each of the methods that the service provides
        // and create an API call method for each.
        const vpnGatewaysStubMethods = ['aggregatedList', 'delete', 'get', 'getStatus', 'insert', 'list', 'setLabels', 'testIamPermissions'];
        for (const methodName of vpnGatewaysStubMethods) {
            const callPromise = this.vpnGatewaysStub.then(stub => (...args) => {
                if (this._terminated) {
    ...
    delete(request, optionsOrCallback, callback) {
        ...
        options.otherArgs.headers['x-goog-request-params'] = this._gaxModule.routingHeader.fromParams({
            ...
            'vpn_gateway': request.vpnGateway ?? '',
        });
    ...
    getStatus(request, optionsOrCallback, callback) {
        request = request || {};
        let options;
        if (typeof optionsOrCallback === 'function' && callback === undefined) {
            callback = optionsOrCallback;
        ...
        this.initialize().catch(err => { throw err; });
        this._log.info('getStatus request %j', request);
        const wrappedCallback = callback
            ? (error, response, options, rawResponse) => {
                this._log.info('getStatus response %j', response);
                callback(error, response, options, rawResponse); // We verified callback above.
            }
            : undefined;
        return this.innerApiCalls.getStatus(request, options, wrappedCallback)
            ?.then(([response, options, rawResponse]) => {
            this._log.info('getStatus response %j', response);
            return [response, options, rawResponse];
        }).catch((error) => {
     ...
     */
    close() {
        if (this.vpnGatewaysStub && !this._terminated) {
            return this.vpnGatewaysStub.then(stub => {
...
//# sourceMappingURL=vpn_gateways_client.js.map