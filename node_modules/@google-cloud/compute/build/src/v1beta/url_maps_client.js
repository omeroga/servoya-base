...
exports.UrlMapsClient = void 0;
const jsonProtos = require("../../protos/protos.json");
const google_gax_1 = require("google-gax");
/**
 * Client JSON configuration object, loaded from
 * `src/v1beta/url_maps_client_config.json`.
 * This file defines retry strategy and timeouts for all API methods in this library.
 */
const gapicConfig = require("./url_maps_client_config.json");
...
class UrlMapsClient {
    ...
    constructor(opts, gaxInstance) {
        ...
        this._defaults = this._gaxGrpc.constructSettings('google.cloud.compute.v1beta.UrlMaps', gapicConfig, opts.clientConfig || {}, { 'x-goog-api-client': clientHeader.join(' ') });
    ...
    initialize() {
        ...
        // google.cloud.compute.v1beta.UrlMaps.
        this.urlMapsStub = this._gaxGrpc.createStub(this._opts.fallback ?
            this._protos.lookupService('google.cloud.compute.v1beta.UrlMaps') :
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            this._protos.google.cloud.compute.v1beta.UrlMaps, this._opts, this._providedCustomServicePath);
        // Iterate over each of the methods that the service provides
        // and create an API call method for each.
        const urlMapsStubMethods = ['aggregatedList', 'delete', 'get', 'insert', 'invalidateCache', 'list', 'patch', 'testIamPermissions', 'update', 'validate'];
        for (const methodName of urlMapsStubMethods) {
            const callPromise = this.urlMapsStub.then(stub => (...args) => {
                if (this._terminated) {
    ...
    invalidateCache(request, optionsOrCallback, callback) {
        request = request || {};
        let options;
        if (typeof optionsOrCallback === 'function' && callback === undefined) {
            callback = optionsOrCallback;
        ...
        this.initialize().catch(err => { throw err; });
        this._log.info('invalidateCache request %j', request);
        const wrappedCallback = callback
            ? (error, response, nextRequest, rawResponse) => {
                this._log.info('invalidateCache response %j', rawResponse);
                callback(error, response, nextRequest, rawResponse); // We verified `callback` above.
            }
            : undefined;
        return this.innerApiCalls.invalidateCache(request, options, wrappedCallback)
            ?.then(([response, operation, rawResponse]) => {
            return [
                { latestResponse: response, done: false, name: response.id, metadata: null, result: {} },
                operation,
    ...
    validate(request, optionsOrCallback, callback) {
        request = request || {};
        let options;
        if (typeof optionsOrCallback === 'function' && callback === undefined) {
            callback = optionsOrCallback;
        ...
        this.initialize().catch(err => { throw err; });
        this._log.info('validate request %j', request);
        const wrappedCallback = callback
            ? (error, response, options, rawResponse) => {
                this._log.info('validate response %j', response);
                callback(error, response, options, rawResponse); // We verified callback above.
            }
            : undefined;
        return this.innerApiCalls.validate(request, options, wrappedCallback)
            ?.then(([response, options, rawResponse]) => {
            this._log.info('validate response %j', response);
            return [response, options, rawResponse];
        }).catch((error) => {
...
//# sourceMappingURL=url_maps_client.js.map