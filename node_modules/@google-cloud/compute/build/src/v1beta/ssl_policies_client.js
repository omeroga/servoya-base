...
exports.SslPoliciesClient = void 0;
const jsonProtos = require("../../protos/protos.json");
const google_gax_1 = require("google-gax");
/**
 * Client JSON configuration object, loaded from
 * `src/v1beta/ssl_policies_client_config.json`.
 * This file defines retry strategy and timeouts for all API methods in this library.
 */
const gapicConfig = require("./ssl_policies_client_config.json");
...
class SslPoliciesClient {
    ...
    constructor(opts, gaxInstance) {
        ...
        this._defaults = this._gaxGrpc.constructSettings('google.cloud.compute.v1beta.SslPolicies', gapicConfig, opts.clientConfig || {}, { 'x-goog-api-client': clientHeader.join(' ') });
    ...
    initialize() {
        ...
        // google.cloud.compute.v1beta.SslPolicies.
        this.sslPoliciesStub = this._gaxGrpc.createStub(this._opts.fallback ?
            this._protos.lookupService('google.cloud.compute.v1beta.SslPolicies') :
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            this._protos.google.cloud.compute.v1beta.SslPolicies, this._opts, this._providedCustomServicePath);
        // Iterate over each of the methods that the service provides
        // and create an API call method for each.
        const sslPoliciesStubMethods = ['aggregatedList', 'delete', 'get', 'insert', 'list', 'listAvailableFeatures', 'patch', 'testIamPermissions'];
        for (const methodName of sslPoliciesStubMethods) {
            const callPromise = this.sslPoliciesStub.then(stub => (...args) => {
                if (this._terminated) {
    ...
    listAvailableFeatures(request, optionsOrCallback, callback) {
        request = request || {};
        let options;
        if (typeof optionsOrCallback === 'function' && callback === undefined) {
            callback = optionsOrCallback;
        ...
        this.initialize().catch(err => { throw err; });
        this._log.info('listAvailableFeatures request %j', request);
        const wrappedCallback = callback
            ? (error, response, options, rawResponse) => {
                this._log.info('listAvailableFeatures response %j', response);
                callback(error, response, options, rawResponse); // We verified callback above.
            }
            : undefined;
        return this.innerApiCalls.listAvailableFeatures(request, options, wrappedCallback)
            ?.then(([response, options, rawResponse]) => {
            this._log.info('listAvailableFeatures response %j', response);
            return [response, options, rawResponse];
        }).catch((error) => {
...
//# sourceMappingURL=ssl_policies_client.js.map