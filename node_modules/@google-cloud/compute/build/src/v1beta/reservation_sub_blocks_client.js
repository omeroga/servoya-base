...
exports.ReservationSubBlocksClient = void 0;
const jsonProtos = require("../../protos/protos.json");
const google_gax_1 = require("google-gax");
/**
 * Client JSON configuration object, loaded from
 * `src/v1beta/reservation_sub_blocks_client_config.json`.
 * This file defines retry strategy and timeouts for all API methods in this library.
 */
const gapicConfig = require("./reservation_sub_blocks_client_config.json");
...
class ReservationSubBlocksClient {
    ...
    constructor(opts, gaxInstance) {
        ...
        this._defaults = this._gaxGrpc.constructSettings('google.cloud.compute.v1beta.ReservationSubBlocks', gapicConfig, opts.clientConfig || {}, { 'x-goog-api-client': clientHeader.join(' ') });
    ...
    initialize() {
        ...
        if (this.reservationSubBlocksStub) {
            return this.reservationSubBlocksStub;
        }
        // Put together the "service stub" for
        // google.cloud.compute.v1beta.ReservationSubBlocks.
        this.reservationSubBlocksStub = this._gaxGrpc.createStub(this._opts.fallback ?
            this._protos.lookupService('google.cloud.compute.v1beta.ReservationSubBlocks') :
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            this._protos.google.cloud.compute.v1beta.ReservationSubBlocks, this._opts, this._providedCustomServicePath);
        // Iterate over each of the methods that the service provides
        // and create an API call method for each.
        const reservationSubBlocksStubMethods = ['get', 'getIamPolicy', 'list', 'performMaintenance', 'reportFaulty', 'setIamPolicy', 'testIamPermissions'];
        for (const methodName of reservationSubBlocksStubMethods) {
            const callPromise = this.reservationSubBlocksStub.then(stub => (...args) => {
                if (this._terminated) {
    ...
    reportFaulty(request, optionsOrCallback, callback) {
        request = request || {};
        let options;
        if (typeof optionsOrCallback === 'function' && callback === undefined) {
            callback = optionsOrCallback;
        ...
        this.initialize().catch(err => { throw err; });
        this._log.info('reportFaulty request %j', request);
        const wrappedCallback = callback
            ? (error, response, nextRequest, rawResponse) => {
                this._log.info('reportFaulty response %j', rawResponse);
                callback(error, response, nextRequest, rawResponse); // We verified `callback` above.
            }
            : undefined;
        return this.innerApiCalls.reportFaulty(request, options, wrappedCallback)
            ?.then(([response, operation, rawResponse]) => {
            return [
                { latestResponse: response, done: false, name: response.id, metadata: null, result: {} },
                operation,
     ...
     */
    close() {
        if (this.reservationSubBlocksStub && !this._terminated) {
            return this.reservationSubBlocksStub.then(stub => {
...
//# sourceMappingURL=reservation_sub_blocks_client.js.map