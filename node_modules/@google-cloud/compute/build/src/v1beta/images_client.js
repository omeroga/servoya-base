 ...
 * `src/v1beta/images_client_config.json`.
 * This file defines retry strategy and timeouts for all API methods in this library.
 */
const gapicConfig = require("./images_client_config.json");
...
class ImagesClient {
    ...
    constructor(opts, gaxInstance) {
        ...
        this._defaults = this._gaxGrpc.constructSettings('google.cloud.compute.v1beta.Images', gapicConfig, opts.clientConfig || {}, { 'x-goog-api-client': clientHeader.join(' ') });
    ...
    initialize() {
        ...
        if (this.imagesStub) {
            return this.imagesStub;
        }
        // Put together the "service stub" for
        // google.cloud.compute.v1beta.Images.
        this.imagesStub = this._gaxGrpc.createStub(this._opts.fallback ?
            this._protos.lookupService('google.cloud.compute.v1beta.Images') :
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            this._protos.google.cloud.compute.v1beta.Images, this._opts, this._providedCustomServicePath);
        // Iterate over each of the methods that the service provides
        // and create an API call method for each.
        const imagesStubMethods = ['delete', 'deprecate', 'get', 'getFromFamily', 'getIamPolicy', 'insert', 'list', 'patch', 'setIamPolicy', 'setLabels', 'testIamPermissions'];
        for (const methodName of imagesStubMethods) {
            const callPromise = this.imagesStub.then(stub => (...args) => {
                if (this._terminated) {
    ...
    deprecate(request, optionsOrCallback, callback) {
        request = request || {};
        let options;
        if (typeof optionsOrCallback === 'function' && callback === undefined) {
            callback = optionsOrCallback;
        ...
        this.initialize().catch(err => { throw err; });
        this._log.info('deprecate request %j', request);
        const wrappedCallback = callback
            ? (error, response, nextRequest, rawResponse) => {
                this._log.info('deprecate response %j', rawResponse);
                callback(error, response, nextRequest, rawResponse); // We verified `callback` above.
            }
            : undefined;
        return this.innerApiCalls.deprecate(request, options, wrappedCallback)
            ?.then(([response, operation, rawResponse]) => {
            return [
                { latestResponse: response, done: false, name: response.id, metadata: null, result: {} },
                operation,
    ...
    getFromFamily(request, optionsOrCallback, callback) {
        request = request || {};
        let options;
        if (typeof optionsOrCallback === 'function' && callback === undefined) {
            callback = optionsOrCallback;
        ...
        this.initialize().catch(err => { throw err; });
        this._log.info('getFromFamily request %j', request);
        const wrappedCallback = callback
            ? (error, response, options, rawResponse) => {
                this._log.info('getFromFamily response %j', response);
                callback(error, response, options, rawResponse); // We verified callback above.
            }
            : undefined;
        return this.innerApiCalls.getFromFamily(request, options, wrappedCallback)
            ?.then(([response, options, rawResponse]) => {
            this._log.info('getFromFamily response %j', response);
            return [response, options, rawResponse];
        }).catch((error) => {
     ...
     */
    close() {
        if (this.imagesStub && !this._terminated) {
            return this.imagesStub.then(stub => {
...
//# sourceMappingURL=images_client.js.map