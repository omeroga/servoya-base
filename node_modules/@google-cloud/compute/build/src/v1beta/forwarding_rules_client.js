...
exports.ForwardingRulesClient = void 0;
const jsonProtos = require("../../protos/protos.json");
const google_gax_1 = require("google-gax");
/**
 * Client JSON configuration object, loaded from
 * `src/v1beta/forwarding_rules_client_config.json`.
 * This file defines retry strategy and timeouts for all API methods in this library.
 */
const gapicConfig = require("./forwarding_rules_client_config.json");
...
class ForwardingRulesClient {
    ...
    constructor(opts, gaxInstance) {
        ...
        this._defaults = this._gaxGrpc.constructSettings('google.cloud.compute.v1beta.ForwardingRules', gapicConfig, opts.clientConfig || {}, { 'x-goog-api-client': clientHeader.join(' ') });
    ...
    initialize() {
        ...
        // google.cloud.compute.v1beta.ForwardingRules.
        this.forwardingRulesStub = this._gaxGrpc.createStub(this._opts.fallback ?
            this._protos.lookupService('google.cloud.compute.v1beta.ForwardingRules') :
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            this._protos.google.cloud.compute.v1beta.ForwardingRules, this._opts, this._providedCustomServicePath);
        // Iterate over each of the methods that the service provides
        // and create an API call method for each.
        const forwardingRulesStubMethods = ['aggregatedList', 'delete', 'get', 'insert', 'list', 'patch', 'setLabels', 'setTarget', 'testIamPermissions'];
        for (const methodName of forwardingRulesStubMethods) {
            const callPromise = this.forwardingRulesStub.then(stub => (...args) => {
                if (this._terminated) {
    ...
    setTarget(request, optionsOrCallback, callback) {
        request = request || {};
        let options;
        if (typeof optionsOrCallback === 'function' && callback === undefined) {
            callback = optionsOrCallback;
        ...
        const wrappedCallback = callback
            ? (error, response, nextRequest, rawResponse) => {
                this._log.info('setTarget response %j', rawResponse);
                callback(error, response, nextRequest, rawResponse); // We verified `callback` above.
            }
            : undefined;
        return this.innerApiCalls.setTarget(request, options, wrappedCallback)
            ?.then(([response, operation, rawResponse]) => {
            return [
                { latestResponse: response, done: false, name: response.id, metadata: null, result: {} },
                operation,
...
//# sourceMappingURL=forwarding_rules_client.js.map